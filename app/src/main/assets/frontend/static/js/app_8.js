...
function _dbgConstruirPanel() {
    ...
    <div id="dbg-bar" style="display:flex;align-items:center;gap:4px;padding:3px 6px;
         background:#0f172a;border-bottom:1px solid #1e293b;cursor:pointer;flex-shrink:0;
         min-width:0;overflow:hidden"
         onclick="_dbgToggleExpand()">

        <span style="color:#4ade80;font-weight:700;font-size:10px;white-space:nowrap;flex-shrink:0">🩺 DEBUG</span>

        <!-- Contadores -->
        <span id="dbg-cnt-err"  style="background:#dc2626;color:#fff;border-radius:999px;
              padding:1px 5px;font-size:9px;display:none;white-space:nowrap;flex-shrink:0">0 err</span>
        <span id="dbg-cnt-warn" style="background:#d97706;color:#fff;border-radius:999px;
              padding:1px 5px;font-size:9px;display:none;white-space:nowrap;flex-shrink:0">0 av</span>
        <span id="dbg-sup-status" style="background:#1e3a5f;color:#38bdf8;border-radius:999px;
              padding:1px 5px;font-size:9px;white-space:nowrap;flex-shrink:0;
              max-width:90px;overflow:hidden;text-overflow:ellipsis">☁️ —</span>

        <!-- Solo 3 botones esenciales en la barra -->
        <div style="margin-left:auto;display:flex;gap:3px;flex-shrink:0">
            <button onclick="event.stopPropagation();_dbgDiagnosticar()"
                title="Diagnóstico completo"
                style="${_dbgBtnStyle('#1d4ed8')}">🔍</button>
            <button onclick="event.stopPropagation();_dbgLimpiar()"
                title="Limpiar log"
                style="${_dbgBtnStyle('#374151')}">🗑️</button>
    ...
    <!-- TABS + ACCIONES secundarias en segunda fila -->
    <div id="dbg-tabs" style="display:flex;flex-wrap:nowrap;background:#0f172a;
         border-bottom:1px solid #1e293b;flex-shrink:0;overflow-x:auto;
        ...
        <!-- Divisor -->
        <span style="width:1px;background:#1e293b;margin:3px 2px;flex-shrink:0"></span>
        <!-- Acciones secundarias -->
        <button onclick="event.stopPropagation();_dbgSetupSupabase()"
            title="Setup Supabase"
            style="${_dbgBtnStyle('#065f46')};margin:2px 1px">☁️ Setup</button>
        <button onclick="event.stopPropagation();_dbgGuardarHistorialHoy()"
            title="Snapshot del día"
            style="${_dbgBtnStyle('#4c1d95')};margin:2px 1px">📅 Snap</button>
        <button onclick="event.stopPropagation();_dbgVerHistorial()"
            title="Ver historial"
            style="${_dbgBtnStyle('#7c3aed')};margin:2px 1px">📋 Hist</button>
    </div>
...
function _dbgBtnStyle(bg) {
    return `background:${bg};color:#e2e8f0;border:none;padding:2px 6px;
...
function _dbgTabStyle(activa) {
    return `background:${activa ? '#1e3a5f' : 'transparent'};color:${activa ? '#7dd3fc' : '#64748b'};
            border:none;padding:4px 8px;cursor:pointer;font-size:11px;font-family:monospace;
...
function _dbgToggleExpand() {
    ...
    // Padding para no tapar contenido de la app
    const app = document.getElementById('tpv-app');
    if (app) app.style.paddingBottom = window._DBG.expanded ? '58vh' : '42px';
}
...
window.tpvDebugger = {
    ...
    activar() {
        ...
        if (panelExiste) {
            // Si está colapsado: expandir. Si ya expandido: solo asegurarse de que se ve.
            if (!window._DBG.expanded) {
                _dbgToggleExpand(); // abre
            } else {
                // Ya está abierto, simplemente aseguramos visibilidad
                const p = document.getElementById('dbg-v2');
                if (p) p.style.height = '55vh';
            }
            return;
...
/* FIX: Botón flotante de debug más visible en móvil */
#dbg-float-btn {
    position: fixed;
    bottom: 80px;
    right: 16px;
    ...
    display: none; /* solo visible si el usuario es dev/admin */
}
#dbg-float-btn.visible { display: flex; align-items: center; justify-content: center; }


// === AUTO-SHOW DEBUG PANEL PARA DEV/ADMIN ===
(function() {
    function autoShowDebug() {
        // Esperar a que la sesión se establezca
        setTimeout(function() {
            fetch('/api/auth/me', {credentials: 'same-origin'})
                .then(r => r.ok ? r.json() : null)
                .then(data => {
                    if (!data || !data.autenticado) return;
                    const rol = data.usuario?.rol;
                    if (rol !== 'desarrollador' && rol !== 'administrador') return;
                    // Mostrar debug panel automáticamente
                    if (typeof window.tpvDebugger === 'object' && window.tpvDebugger.activar) {
                        window.tpvDebugger.activar();
                        console.log('[DEBUG] Panel auto-activado para rol:', rol);
                    } else if (typeof window._dbg_mostrar === 'function') {
                        window._dbg_mostrar();
                        console.log('[DEBUG] Panel auto-activado (legacy)');
                    }
                })
                .catch(e => console.log('[DEBUG] No se pudo auto-activar:', e));
        }, 2000); // 2 segundos después de cargar
    }
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        autoShowDebug();
    } else {
        document.addEventListener('DOMContentLoaded', autoShowDebug);
    }
})();


// === DEBUG PANEL v5 — EXCLUSIVO desarrollador ===
(function() {
    'use strict';

    function getRole() {
        try {
            if (window.AUTH && AUTH.usuario && AUTH.usuario.rol) return AUTH.usuario.rol;
            if (window.tpvState && tpvState.usuarioActual && tpvState.usuarioActual.rol) return tpvState.usuarioActual.rol;
        } catch (e) {}
        return null;
    }

    function isDev() {
        return getRole() === 'desarrollador';
    }

    function toggleDebug() {
        if (!isDev()) {
            console.warn('[DEBUG] Denegado. Rol actual:', getRole(), '(requiere: desarrollador)');
            return;
        }
        console.log('[DEBUG] Toggle — rol:', getRole());

        // Método 1: tpvDebugger (sistema original)
        if (window.tpvDebugger && typeof window.tpvDebugger.activar === 'function') {
            window.tpvDebugger.activar();
            console.log('[DEBUG] Activado vía tpvDebugger');
            return;
        }
        // Método 2: _dbg_mostrar (legacy)
        if (typeof window._dbg_mostrar === 'function') {
            window._dbg_mostrar();
            console.log('[DEBUG] Activado vía _dbg_mostrar');
            return;
        }
        // Método 3: toggle manual del panel dbg-v2
        var panel = document.getElementById('dbg-v2');
        if (panel) {
            var h = panel.style.height;
            var isCollapsed = (h === '38px' || h === '' || !h);
            panel.style.height = isCollapsed ? '55vh' : '38px';
            var content = document.getElementById('dbg-content');
            if (content) content.style.display = isCollapsed ? '' : 'none';
            var tabs = document.getElementById('dbg-tabs');
            if (tabs) tabs.style.display = isCollapsed ? 'flex' : 'none';
            console.log('[DEBUG] Panel toggled:', isCollapsed ? 'ABIERTO' : 'cerrado');
        } else {
            console.warn('[DEBUG] Panel dbg-v2 no encontrado. ¿app_8.js se cargó completo?');
        }
    }

    function showDebugButton() {
        var btn = document.getElementById('dbg-float-btn');
        if (!btn) {
            createDebugButton();
            btn = document.getElementById('dbg-float-btn');
            if (!btn) return;
        }
        if (isDev()) {
            btn.style.display = 'flex';
            if (!window._debugBtnLogged) {
                console.log('[DEBUG] 🩺 Botón visible para desarrollador');
                window._debugBtnLogged = true;
            }
        } else {
    ...
    function createDebugButton() {
        if (document.getElementById('dbg-float-btn')) return;
        var btn = document.createElement('button');
        btn.id = 'dbg-float-btn';
        btn.innerHTML = '🩺';
        btn.title = 'Debug Panel (F1) — Solo desarrollador';
        btn.style.cssText = [
            'position:fixed', 'bottom:88px', 'left:16px', 'z-index:999998',
        ...
        console.log('[DEBUG] Botón 🩺 creado (violeta, exclusivo dev)');
    }
    ...
    // Exponer globalmente
    window.tpvDebugToggle = toggleDebug;
    window.tpvDebugCheck = showDebugButton;
    window.tpvDebugIsDev = isDev;

    function init() {
        createDebugButton();
        showDebugButton();
        console.log('[DEBUG] Sistema debug v5 listo (exclusivo desarrollador)');
    }
    ...
    // Re-check cada 1.5s (más frecuente para detectar login rápido)
    setInterval(showDebugButton, 1500);
})();
