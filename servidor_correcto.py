...
# ========== SERVIR TU INDEX ORIGINAL ==========
@app.route('/static/<path:filename>')
def serve_static(filename):
    """Sirve archivos estáticos desde app/src/main/static/"""
    file_path = os.path.join(f"{base_dir}/app/src/main/static", filename)
    if os.path.exists(file_path):
        return send_file(file_path)
    
    # Buscar en subcarpetas
    for sub in ['css', 'js', 'lib', 'fonts']:
        file_path = os.path.join(f"{base_dir}/app/src/main/static", sub, filename)
        if os.path.exists(file_path):
            return send_file(file_path)
...
def index():
    ...
    return "Index no encontrado. Los archivos estáticos no están en la ubicación correcta.", 404

if __name__ == '__main__':
    print("="*60)
    print("🚀 TPV - CON TU DISEÑO ORIGINAL")
    print("="*60)
